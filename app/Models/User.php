...
class User extends Authenticatable implements HasMedia, NotificationTarget
{
    ...
    public function ownedBuildings()
    {
     ...
     */
    public function addTransaction($amount, array $data = []): Transaction
    {
        ...
        if ($transaction->model instanceof Rent) {
            $model = $transaction->model;

            $availableAmount = $amount;

            $unpaid = $model->installments()->partiallyOrUnpaid()->first();

            while ($availableAmount && $unpaid) {
                if ($availableAmount <= $unpaid->remaining_amount) {
                    $transactionDetails = $transaction->details()->create([
                        'amount' => $availableAmount,
                    ]);
                    $unpaid->forceFill([
                        'paid_amount' => ((float) $unpaid->paid_amount) + $availableAmount,
                    ...
                    $transactionDetails = $transaction->details()->create([
                        'amount' => $remaining = $unpaid->remaining_amount,
                    ]);
                    $unpaid->forceFill([
                        'paid_amount' => ((float) $unpaid->paid_amount) + $unpaid->remaining_amount,
                        'transaction_detail_id' => $transactionDetails->id,
                    ])->save();
                    if ($unpaid = $model->installments()->partiallyOrUnpaid()->first()) {
                        $availableAmount = $availableAmount - $remaining;
                    }
                }
            }
        } else {
